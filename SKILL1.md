...
5. **The uploaded donor file is the single source of truth.** Never use donor
   data recalled from memory, from a prior conversation, or from anywhere
...
donor. It parses giving history, derives lifetime total, largest single gift and
last gift year where those columns are absent, reads the tier, computes the ask
amount, sets the loyalty and volunteer flags, detects consecutive giving
streaks, and routes any bad row to an exceptions file. Output is one structured
...
letter gets a single completeness check rather than a field by field
verification. A letter that fails the check goes to exceptions and the rest of
the run continues.
...
| largest_gift               | No       | Derived from gifts when absent                 |
| lifetime_total             | No       | Derived from gifts when absent                 |
| last_gift_year             | No       | Derived from gifts when absent                 |
...
- **Trust the `tier` column.** It is the organization's own classification and
  is authoritative, including for donors whose giving totals would otherwise
  suggest a different band. Do not recompute or override it.
...
After rendering each letter, run one check rather than verifying fields
individually: scan the output for any surviving `[` `]` token or unresolved
`{{` marker. Clean output means the letter is complete. A letter that fails goes
