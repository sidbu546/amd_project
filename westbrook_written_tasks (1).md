...
- The actual log (Artifact E) shows something completely different. Ten of twelve requests closed in **5 to 13 minutes**, median about 9 minutes. Two closed in 3 days 4 hours and 1 day 18 hours. **The mean is an artifact of two outliers.** There is no general slowness problem. There is a tail problem.
...
**Worth:** trivial in labour terms. Nine minutes times 1,840 is not a business case. The value is in the tail — an attorney's request sitting eleven days, and the reputational and possible compliance exposure that carries. **I would do it in week one anyway**, because it costs almost nothing, it visibly fixes something the staff have complained about for months, and it buys the credibility I will need in week six when I ask them to change how authorizations are handled.

**Note the discrepancy:** the twelve-request sample averages roughly 10 hours, not 4.2. Either the sample is unrepresentative, or the log measures business hours while my arithmetic measures wall clock. I need to ask.
...
Contradicted by the log the average was drawn from. Median is ~9 minutes; the mean is created by two multi-day outliers.
*Working from:* median ~9 minutes with a heavy tail. This changes the intervention completely — from "make requests faster," which would have been an automation project, to "stop requests from falling into holes," which is an inbox and an owner.
...
**Where I used AI.** Drafting and compression, mostly. Getting the first version of the triage structure onto the page, tightening the build specification into acceptance criteria that could actually be tested, generating the Mermaid syntax for the current-state diagram, and pressure-testing my ranking by asking for the strongest argument that referral conversion should be first instead of prior auth. That last one changed the document: it is why the ranking says explicitly that referrals become #1 if the day-30 baseline comes in below 70 percent, rather than leaving the order looking more settled than the evidence supports.

**What I kept human.** Every judgement. The ranking basis, the decision to leave the eight high-volume clinicians alone, the reading of Artifact E as a tail problem rather than a speed problem, the decision to make the undefined denominator the central slide of the deck, and every number I chose to commit to. The failure modes in section 4.7 came from my own experience of what breaks; a model will reliably produce a generic list of risks that reads well and warns you about nothing in particular.

**What I checked, and what I found.** I recomputed the records-request log by hand rather than trusting a summary — that is how the 4.2-hour figure came apart, and how I noticed the twelve-request sample does not reconcile with it either. I traced every quantitative claim in this document back to the artifact it came from and marked the assumptions, which is why the operations-savings figure carries an explicit warning about the auth-team headcount I inferred. I checked the arithmetic on the 20 percent target three times, because it is the number I am most likely to be wrong about in front of a CEO.

**What I would not use AI for on this engagement, and told the client so.** Anything that puts patient information into a tool without a BAA, which is the entire substance of Alicia's note and the reason a shadow-IT inventory sits in week one of the plan. Every artifact in this pack is fictional, so nothing here carried PHI. On a real Westbrook engagement, that constraint governs the tooling from day one and it is not negotiable for convenience.
