...
## 4. The donor list that is hardcoded into the skill file

**Original:** A table of donor names, giving histories, lifetime totals and
volunteer status, embedded in the instructions and offered as a data source
alongside an uploaded CSV.

**Why this blocks the brief directly:** The requirement is a skill that scales
to a growing donor list. A donor table living inside the prompt is the opposite.
It is stale the moment a gift is recorded, it duplicates donor PII into every
conversation where the skill loads, and growing the donor base means editing the
skill file itself.

**The context window makes this a hard ceiling, not just an inconvenience.** An
AI assistant reads everything it is given through a fixed size context window,
and a skill's instructions are loaded into that window every time the skill
triggers. So a donor table pasted into `SKILL.md` occupies the context on every
single run, in full, whether or not any of those donors appear in today's
campaign. A short list is survivable. A real donor base is not, and past a
certain size the skill simply stops fitting.
...
| Donor data | Donor list hardcoded in the prompt | Uploaded file is the only source |
